...
public class SinglePageRender extends PageRender {
    ...
    SinglePageRender(Context context, PageFlip pageFlip,
                     Handler handler, int pageNo, ArrayList<String> pages) {
        super(context, pageFlip, handler, pageNo, pages.size());
        pics = pages;
    }
    ...
    public void onDrawFrame() {
        ...
        mPageFlip.enableClickToFlip(false);
    ...
    private void drawPage(int number) {
        ...
        try {
            background = MediaStore.Images.Media.getBitmap(mContext.getContentResolver(), Uri.parse(pics.get((number - 1) % pics.size())));
